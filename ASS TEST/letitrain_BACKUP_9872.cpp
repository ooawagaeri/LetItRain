...
void Letitrain::initialize(HWND hwnd)
{
  ...
  if (!map.initialize(this,0,0,mapNS::TEXTURE_COLS,&mapTexture))
	...
	// enemyShip textures
	if (!gruntTexture.initialize(graphics, ENEMY_IMAGE))
		 throw(GameError(gameErrorNS::FATAL_ERROR, "Error initializing game textures"));
	...
	// enemyShip
	if (!enemyList.initialize(this, &gruntTexture))
...
void Letitrain::update()
{
	...
	enemyList.update(frameTime);
  pwM.update(frameTime,bulletship, lasership);
  bulletList.update(frameTime, input, this, bulletNS::WIDTH, bulletNS::HEIGHT, bulletNS::BULLET_TEXTURE_COLS, &weaponTextures,bulletship.getX()+20,bulletship.getY());
	laserList.update(frameTime, input, this, laserNS::WIDTH, laserNS::HEIGHT, laserNS::LASER_TEXTURE_COLS, &weaponTextures, lasership.getX()+25, lasership.getY());
}
...
void Letitrain::ai()
{
	enemyList.ai(&bulletship, &lasership);
}
...
=======
	enemyList.collisions(&bulletship, &lasership);
>>>>>>> Stashed changes
