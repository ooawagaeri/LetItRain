...
bool Entity::DetectPixelPerfect(Entity &ent, VECTOR2 &collisionVector)
{
	...
	if (IntersectRect(&dest, &rect1, &rect2))
	{
		...
		HRESULT hResult = spriteData.texture->LockRect(0, &rectS1, NULL, 0);
		if (FAILED(hResult))
		{
		...
		BYTE* pixelsS1 = (BYTE*)rectS1.pBits;
		BYTE* pixelsS2 = (BYTE*)rectS2.pBits;
		...
		for (int rx = dest.left; rx < dest.right; rx++)
		{
			for (int ry = dest.top; ry < dest.bottom; ry++)
			{
				...
				int index1 = (s1x * 4 + (s1y*(rectS1.Pitch)));
				int index2 = (s2x * 4 + (s2y*(rectS2.Pitch)));
				...
				BYTE c = pixelsS1[index1 + 3];
				BYTE d = pixelsS1[index2 + 3];

				BYTE e = ExtractAlpha(pixelsS1[index1]);
				BYTE f = ExtractAlpha(pixelsS2[index2]);

				// If both pixels are opaque, we found a collision
				if (a == 255 && b == 255)
				{
					spriteData.texture->UnlockRect(0);
...
bool Entity::collideRotatedBoxCircle(Entity &ent, VECTOR2 &collisionVector)
{
    ...
    collisionVector = *ent.getCenter() - *getCenter();
	return DetectPixelPerfect(ent, collisionVector);
}
